...
def create_excel(findings):
    ...
    for vuln_name in sorted_vulns:

        entries = findings[vuln_name]

        first = True

        for file_path, line_no, severity, matched_text in entries:
    ...
    with open(
        "output\\Advanced_Rules_Engine_Findings.csv",
        "w",
        newline="",
        encoding="utf-8"
    ) as review:
        ...
        for vuln_name, entries in findings.items():
    ...
    wb.save("output\\Advanced_Rules_Engine.xlsx")
...
def main():
    ...
    success("Advanced_Rules_Engine.xlsx generated successfully")
    success("Advanced_Rules_Engine_Findings.csv generated successfully")
